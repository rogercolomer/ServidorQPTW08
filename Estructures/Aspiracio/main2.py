...
class Bucle:
    ...
    def decode_alarm(self):
        ...
        for k in self.alarm_list:
            for j in range(7, -1, -1):
                if self.alarms[k].get_alarm()[j] == '1':
                    if 80 < comptador_al+1 < 93 or comptador_al+1 == 34 or comptador_al+1 == 35 or comptador_al+1 == 36 or comptador_al+1 == 37 or comptador_al+1 == 38 or comptador_al+1 == 41:
                        pass
                    else:
        ...
        if len(al_num)<5:
            ...
            al_num = [al_num[0], al_num[1], al_num[2], al_num[3], al_num[4]]
        self.alarm_code = al_num
        # print(self.alarm_code,al_num)
    ...
    def save_master(self):
        ...
        for a in self.alarms:
            print(a)
            if a == 'a10':
                if self.alarms[a].get_alarm() != '00000000' and self.alarms[a].get_alarm() != '00000001':
                    self.save_al_code('192.100.101.40')
                    break
                elif self.alarms[a].get_alarm() == '00000000' or self.alarms[a].get_alarm() == '00000001':
                    compNoAlarm += 1
            else:
                if self.alarms[a].get_alarm() != '00000000':
                    self.save_al_code('192.100.101.40')
                    break
                elif self.alarms[a].get_alarm() == '00000000':
                    compNoAlarm += 1
        if compNoAlarm == 37:
            self.save_al_code('192.100.101.40')
