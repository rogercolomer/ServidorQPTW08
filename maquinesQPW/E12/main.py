...
from machineIOT import ModbusNX
        ...
        if self.varCom['protocol'] == 'ModbusTCP':
            ...
            if self.lecturaDada():
                if self.registreEstat == 1:
                    ...
                    self.bitParada = 1
                    self.saveFisico()
        elif self.varCom['protocol'] == 'ModbusNX':
            self.registre = ModbusNX(self.varCom['variables'], host=self.varCom['ip'])
            print('modbusNX')
            if self.lecturaDada():
                if self.registreEstat == 1:
                    self.EstadoLinea = 1
