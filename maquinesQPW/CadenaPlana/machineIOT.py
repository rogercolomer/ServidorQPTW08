...
class UART(Comunication):
    ...
    def readData(self):
        ...
        if a:
            if (len(a) - 7) == (
                    self.len0 * 4):  # Despreciem 7 bits per comprovar que la trama és de la mida que esparem
                for j in range(2, len(a) - 5, 4):  # recorrem la trama des del bit 2 fins lúltim -5 de 4 en 4
                    b = ''
                    for k in range(4):
                        b = b + a[j + k]
                    self.data.append(int('0x' + b, 16))  # Guardem els enters en una llista
        self.makeArray()
        self.makeBytes()
        return True
        # except:
...
class Modbus(Comunication):
    ...
    def readData(self):
        self.client.connect()
        try:
            rr = self.client.read_holding_registers(0x000, self.len0 + 5, unit=0)  # adressa, numero , nideia123
...
class S7(Comunication):
    ...
    def readData(self):
        ...
        if connection == 1:
            self.data = []
            value = plc.db_read(self.db, 0, self.len0)  # llegir db (numero_db,primer byte, longitud maxima)
